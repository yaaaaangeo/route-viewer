...
`npm test` 합계 745개(16개 파일, 2026-09-16 실행 전부 통과). 같은 날 브라우저 모드 E2E 19/19 통과,
`node tests/server-sync-test.js` 25/25 통과, 데스크톱 E2E 133개 통과·1개 실패
(건물 데이터를 못 받은 날에는 탭 복귀 재사용 검사 1개를 설계상 건너뜀) —
실패 1건은 아래 알려진 이슈의 "지역별 Coverage 요약 패널"이며, 화면 콘솔 에러 8건은 모두 Overpass 미러 접속 실패 경고입니다. `tests/manual-cells-storage-test.js`는 브라우저 IndexedDB 백엔드를
...
- **교통 시간대·조도 조건 — 아직 없는 것**: 공휴일은 평일로 셉니다(요일 구분은 토·일만 주말). 날씨는 파일 원본 문자열
  ...
  추천 기능은 없고 집계 API만 있습니다. 누적 지도(밀도·Coverage)에는 조건 필터가 없습니다.
